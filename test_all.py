"""End-to-end test suite for Digital Signature Tool v2.0."""
import sys
import os
import zipfile
import json
...
print("\n[TEST 1] RSA Key Generation...")
from crypto_engine.key_manager import generate_rsa_keypair, list_keys, get_public_key_pem
meta = generate_rsa_keypair(key_size=2048, key_name="test_rsa_2048")
print(f"  OK Generated RSA 2048-bit key: {meta['key_name']}")
print(f"  OK Fingerprint: {meta['fingerprint']}")

# --- Test 2: ECDSA Key Generation ---
print("\n[TEST 2] ECDSA Key Generation...")
meta2 = generate_rsa_keypair(key_size=2048, key_name="test_rsa_backup")
from crypto_engine.key_manager import generate_ecdsa_keypair
meta_ec = generate_ecdsa_keypair(curve_name="secp256r1", key_name="test_ecdsa_p256")
...
result = sign_file("uploads/test_contract.pdf", "test_rsa_2048")
...
ec_result = sign_file("uploads/test_contract.pdf", "test_ecdsa_p256")
...
wrong_pub = get_public_key_pem("test_rsa_backup")
...
# --- Test 13: Audit report for INVALID bundle ---
print("\n[TEST 13] Audit Report for INVALID bundle...")
invalid_report = generate_audit_report(tv)  # tv = tampered verification result
assert "INVALID" in invalid_report, "FAILED: Invalid report should say INVALID!"
assert "WARNING" in invalid_report, "FAILED: Invalid report should contain WARNING!"
print("  PASS - Invalid bundle report generated correctly!")

# --- Summary ---
print("\n" + "=" * 60)
print("ALL 13 TESTS PASSED! (v2.0)")
print("  - Metadata binding (timestamp + algorithm)")
print("  - Metadata tampering detection")
